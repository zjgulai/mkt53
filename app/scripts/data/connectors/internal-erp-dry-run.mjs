...
function readSnapshotManifest(snapshotManifestPath) {
  ...
  return unwrapJsonObject(payload, 'snapshotManifest');
}
...
function buildInternalErpReadinessGate(dryRun, options = {}, env = process.env) {
  ...
  const readinessPathSource = configuredPathSource({
    ...
    envPath: env.MKT53_ERP_READINESS_PATH,
    envName: 'MKT53_ERP_READINESS_PATH',
  });
  const snapshotManifestPathSource = configuredPathSource({
    explicitPath: options.snapshotManifestPath,
    envPath: env.MKT53_ERP_SNAPSHOT_MANIFEST_PATH,
    envName: 'MKT53_ERP_SNAPSHOT_MANIFEST_PATH',
  });
  ...
  const forbiddenKeyPatterns = [forbiddenSecretKeyPattern, forbiddenCommercialKeyPattern];
  const forbiddenKeys = [...findForbiddenKeys(readiness, forbiddenKeyPatterns), ...findForbiddenKeys(snapshotManifest, forbiddenKeyPatterns)];
