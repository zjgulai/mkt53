...
function buildInternalCrmReadinessGate(dryRun, options = {}, env = process.env) {
  ...
  const readinessPathSource = configuredPathSource({
    ...
    envPath: env.MKT53_CRM_READINESS_PATH,
    envName: 'MKT53_CRM_READINESS_PATH',
  });
  const snapshotManifestPathSource = configuredPathSource({
    explicitPath: options.snapshotManifestPath,
    envPath: env.MKT53_CRM_SNAPSHOT_MANIFEST_PATH,
    envName: 'MKT53_CRM_SNAPSHOT_MANIFEST_PATH',
  });
  ...
  const forbiddenKeyPatterns = [forbiddenSecretKeyPattern, forbiddenPiiKeyPattern];
