...
**执行记录（2026-05-31）:**
- [x] 创建产品路线图，覆盖当前能力、不可信/示例数据、真实数据接入优先级和 90 天路线。
- [x] 创建数据与 AI 代理架构文档，明确静态前端、Backend API、PostgreSQL、对象存储和 AI image proxy 边界。
- [x] 更新根目录 `README.md` 正式文档索引。
- [x] 明确不做项：不在浏览器保存供应商 API key；不把未核实法规显示为已验证事实。
