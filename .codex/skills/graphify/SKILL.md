...
**Step B3 - Validate the in-memory results and cache them (Codex)**

Step B2 has already waited for every subagent and assembled the valid JSON responses in memory. Do not look for `graphify-out/.graphify_chunk_NN.json`; Codex subagents return their payloads inline and do not create those files.

For each returned payload:
- require a JSON object containing `nodes` and `edges`; `hyperedges` may be omitted and defaults to an empty list
- if a subagent failed or returned invalid JSON, print a warning and skip that chunk — do not silently discard it
- if more than half the chunks fail validation, stop and ask the user to re-run after checking the failed subagent responses
- when tool usage metadata is available, use it for `input_tokens` and `output_tokens`; otherwise preserve the counts returned in the payload

Write the accumulated valid results once to `graphify-out/.graphify_semantic_new.json`, with combined `nodes`, `edges`, `hyperedges`, `input_tokens`, and `output_tokens`, then continue with the cache step below.
...
report = generate(G, communities, cohesion, labels, analysis['gods'], analysis['surprises'], detection, tokens, 'INPUT_PATH', suggested_questions=questions, min_community_size=0)
Path('graphify-out/GRAPH_REPORT.md').write_text(report, encoding=\"utf-8\")
Path('graphify-out/.graphify_labels.json').write_text(json.dumps({str(k): v for k, v in labels.items()}, ensure_ascii=False), encoding=\"utf-8\")
Path('graphify-out/.graphify_labels.json.sig').write_text(json.dumps({str(k): v for k, v in community_member_sigs(communities).items()}), encoding=\"utf-8\")
