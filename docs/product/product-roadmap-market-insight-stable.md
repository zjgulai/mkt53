...
| P1 | Amazon 竞品价格与评价 | `needs-review` | 已有 dry-run 输出契约、ASIN/SKU 映射校验和私有映射模板，但缺完整私有映射、平台授权记录和真实采集时间戳 | 补齐私有映射表、授权记录，再实现真实连接器和数据快照 |
...
| P0 | 竞品采集 | 支撑价格与新品监控 | Amazon dry-run 契约 + 私有 ASIN/SKU 映射校验 + SKU 采集快照 | 竞品情报组 | 私有 ASIN/SKU 映射、平台授权、价格、评价、采集时间 | 关键竞品 SKU 映射覆盖率不低于 90%，dry-run 无真实平台请求，真实映射不进入前端包或 git |
