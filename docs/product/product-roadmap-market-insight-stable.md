...
下一阶段按“自动化、模块化、数仓管理专业化”推进。

| 原则 | 执行含义 | 禁止事项 |
|---|---|---|
| 自动化 | 半月刷新、公开证据、连接器 readiness、私有审计、生产 smoke/E2E 必须脚本化并可重复执行 | 不用人工复制 JSON 或手改生产静态文件制造当前状态 |
| 模块化 | 采集、私有输入校验、快照生成、页面展示、文档同步分层实现；每个数据域先形成独立契约 | 不把 Amazon、CRM、ERP、VOC、报告生成揉成一个大脚本 |
| 数仓管理专业化 | 所有真实业务数据进入快照契约、来源 registry、字段口径、采集窗口、脱敏边界和复核记录 | 不把公开网页样本、模型推断或示例数组包装成数仓事实表 |
| 证据优先 | `connector-required`、`manual-required`、`example` 状态只能由授权快照或人工凭证升级 | 不为了页面好看把缺证来源改成已验证 |
| 生产一致性 | `main`、生产 manifest、自动化副本和接手文档必须同步收口 | 不让生产长期领先于 Git 主线 |
...
| 公开证据样本 | 已上线半月 manifest 摘要 | `/#/data-source`、`public-evidence-samples.json` | 只作为公开页面证据索引，不替代 Amazon/CRM/ERP/VOC/海关/访谈连接器或人工凭证 |
...
| P1 | Amazon 竞品价格与评价 | `needs-review` | 已有 dry-run 输出契约、ASIN/SKU 映射校验、67 行私有填报草稿生成器、私有映射 promotion gate、私有 readiness 填报草稿和 promotion gate、私有占位初始化（含 readiness checklist）、私有输入交叉审计、私有覆盖率归档和半月窗口 readiness gate，但缺完整私有映射、平台授权记录和真实采集时间戳 | 按私有填报草稿补齐 ASIN/SKU 映射表和授权记录，先通过 promotion gate 写入正式私有 mapping/readiness，再通过私有输入交叉审计和半月窗口 readiness gate 后实现真实连接器和数据快照 |
...
| P0 | 竞品采集 | 支撑价格与新品监控 | Amazon dry-run 契约 + 67 行私有填报草稿 + 私有映射/readiness promotion gate + 私有输入占位/checklist + 私有输入交叉审计 + 私有 ASIN/SKU 映射覆盖率归档 + readiness gate + SKU 采集快照 | 竞品情报组 | 私有 ASIN/SKU 映射、平台授权、价格、评价、采集时间 | 关键竞品 SKU 映射覆盖率不低于 90%，私有映射和 readiness 均通过 promotion gate 备份并写入后，私有输入交叉审计和 readiness gate 通过才进入真实连接器；覆盖率归档和审计报告留在服务器私有目录，真实映射不进入前端包或 git |
