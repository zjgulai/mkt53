...
3. `npm run data:audit` 出现 critical issue 时，先修复表治理、registry id 或页面映射问题；warning 可进入补齐队列，但必须在周度工作流文档留痕。
4. `npm run test:e2e` 失败时，优先区分测试选择器误判、控制台错误、水平滚动和路由渲染错误。
5. `npm run test:e2e:prod` 失败时，先确认线上 landing 是否被其他服务更新，再判断是页面漂移还是测试期望需要同步。
6. 生产部署前不得跳过 `deploy-static.sh` 内置门禁。
